...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // Only store valid IDs (positive integers)
      if (res.patient_id && Number(res.patient_id) > 0) localStorage.setItem('patient_id', String(res.patient_id))
      if (res.user_id && Number(res.user_id) > 0) localStorage.setItem('user_id', String(res.user_id))
      ...
      if (res.doctor_id && Number(res.doctor_id) > 0) localStorage.setItem('doctor_id', String(res.doctor_id))
      ...
      const dest = r === 'DOCTOR' ? '/doctor' : r === 'ADMIN' ? '/admin' : '/'
