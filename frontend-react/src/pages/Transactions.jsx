...
import {
  LuSearch, LuCheck, LuX, LuRefreshCcw, LuWallet, LuCalendar
} from 'react-icons/lu'
import { Card, CardBadge, IconWrapper } from '../components/Card'
...
export default function Transactions() {
  ...
  async function load() {
    ...
    try {
      ...
      const sum = await api.transactionsSummary()
      setSummary(sum)
    } catch (err) {
      console.error(err)
    } finally {
  ...
  useEffect(() => { load() }, [filter, start, end, category])
  ...
  return (
    <div className="min-h-screen pb-20 space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">View your consultation and treatment charges</p>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card className="p-5" hover={false}>
            ...
            <p className="text-2xl font-bold text-slate-900">₹ {(summary.pending || 0).toFixed(2)}</p>
          </Card>
          <Card className="p-5" hover={false}>
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-2xl font-bold text-slate-900">₹ {(summary.completed || 0).toFixed(2)}</p>
          </Card>
          <Card className="p-5" hover={false}>
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-2xl font-bold text-slate-900">{summary.count || 0}</p>
          </Card>
        </div>
      )}
      ...
      <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 flex items-center gap-1.5">
                      <span className="capitalize">{t.category || 'consultation'}</span> • <span>{new Date(t.created_at).toLocaleDateString()}</span>
                    </p>
                  </div>
                </div>
        ...
        {loading && <div className="text-center py-10 text-slate-400">Loading billing history...</div>}
        {!loading && list.length === 0 && (
          <div className="text-center py-20 bg-slate-50 rounded-3xl border border-slate-200 border-dashed">
            ...
            <p className="text-slate-400">Your billing history will appear here after consultations.</p>
          </div>
        )}
      </div>
    </div>
